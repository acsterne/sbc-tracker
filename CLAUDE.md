...
- **Stock price enrichment** — `fetch_prices.py` fetches historical year-end stock prices from Yahoo Finance (yfinance). Uses EDGAR companyfacts `end` dates to determine each company's fiscal year end, then looks up the closing price on or before that date. Computes market_cap (shares × price) and sbc_pct_market_cap. Stores in the metrics table. Also split-adjusts historical shares_outstanding using yfinance split history so all share counts are in post-split terms.
- **Buyback share estimation** — the dilution chart estimates shares_repurchased as buyback_spend / stock_price when EDGAR doesn't report the share count directly (common for GOOGL, AMZN, etc.).
